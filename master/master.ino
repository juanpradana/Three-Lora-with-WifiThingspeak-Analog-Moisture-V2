  ...
  if (Sender.available()) {
    ...
    if (timeNow - tempTime > 20) {
      ...
      if (queue == 0 && datas.indexOf("#0") > 0) {
        ...
        String dataCapture = datas.substring(nilaiIndex, (nilaiIndex + 16));
      ...
      else if (queue == 2 && datas.indexOf("#2") > 0) {
        int nilaiIndex = datas.indexOf("User#2: ");
        String dataCapture = datas.substring(nilaiIndex, (nilaiIndex + 16));
        String finalDatas = getValue(dataCapture, '%', 0);
        finalDatas.replace("User#2: ", "");
        Serial.println(finalDatas);
    ...
    Serial.println("time queue: " + String(queue));
  }
  delay(2000);
}
